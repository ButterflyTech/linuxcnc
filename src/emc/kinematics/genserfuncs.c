...
void dbg_dump(void) {
  int t;
  genser_struct *p=KINS_PTR;
  for (t = 0; t < GENSER_MAX_JOINTS; t++) {
    fprintf(stderr,"t=%d a=%8.3f alpha=%8.3f d=%8.3f th=%8.1f type=%d quan=%d \n"
    ,t
    ,p->links[t].u.dh.a
    ,p->links[t].u.dh.alpha* 180/PM_PI
    ,p->links[t].u.dh.d
    ,p->links[t].u.dh.theta
    ,p->links[t].type
    ,p->links[t].quantity
    );
  }
}
void dbg_after_inverse(char*tag,double* joints, EmcPose* world) {
fprintf(stderr,"\n%-5s WORLD:%8.2f %8.2f %8.2f %8.2f %8.2f %8.2f\n"
,tag,world->tran.x,world->tran.y,world->tran.z,world->a,world->b,world->c);
fprintf(stderr,  "%-5sJOINTS:%8.2f %8.2f %8.2f %8.2f %8.2f %8.2f\n"
,tag ,joints[0],joints[1],joints[2],joints[3],joints[4],joints[5]);

EmcPose w;
int r;
KINEMATICS_FORWARD_FLAGS  fflags;
KINEMATICS_INVERSE_FLAGS  iflags;
r= genserKinematicsForward(joints, &w, &fflags, &iflags);

fprintf(stderr,"%-5s  newW:%8.2f %8.2f %8.2f %8.2f %8.2f %8.2f r=%d\n"
,tag,w.tran.x,w.tran.y,w.tran.z,w.a,w.b,w.c,r);
...
int genser_kin_init(void) {
    ...
    static volatile double tst=0;tst=sqrt(tst); // insure -lm used
    ...
    for (genser->iterations = 0;
        ...
        */
        ...
        if (smalls == genser->link_num) {
            ...
            //     joints[0],joints[1],joints[2],joints[3],joints[4],joints[5], genser->iterations);
//dbg_after_inverse("A",joints,world);
            return GO_RESULT_OK;
        }
        /* else keep iterating */
        for (link = 0; link < genser->link_num; link++) {
